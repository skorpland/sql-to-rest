...
export type PowerbaseJsQuery = {
  code: string
}

/**
 * Renders a `Statement` as a powerbase-js query.
 */
export async function renderPowerbaseJs(processed: Statement): Promise<PowerbaseJsQuery> {
  switch (processed.type) {
    case 'select':
      return formatSelect(processed)
    default:
      throw new RenderError(`Unsupported statement type '${processed.type}'`, 'powerbase-js')
  }
}

async function formatSelect(select: Select): Promise<PowerbaseJsQuery> {
  const { from, targets, filter, sorts, limit } = select
  const lines = ['const { data, error } = await powerbase', `.from('${from}')`]
  ...
  if (limit) {
    if (limit.count !== undefined && limit.offset === undefined) {
      ...
      throw new RenderError(`powerbase-js doesn't support an offset without a limit`, 'powerbase-js')
    } else if (limit.count !== undefined && limit.offset !== undefined) {
...
function renderFilterRoot(lines: string[], filter: Filter) {
  ...
  if (filter.negate) {
    ...
    // powerbase-js doesn't support negated logical operators.
  ...
  // Logical operator filter, eg. .or('title.eq.Cheese,title.eq.Salsa')
    ...
    throw new RenderError(`Unknown filter type '${type}'`, 'powerbase-js')
  }
}
